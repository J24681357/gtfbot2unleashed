...
module.exports.startsession = function (location, msg, embed, userdata) {
  ...
  var results = ""
  var usercar = {x:1,y:3, object:gtftools.toEmoji("playercar")}
...
r3 = gtftools.randomInt(0,100)
      ...
      if (currpattern["pattern"].length == 0 && allobjects.filter(x => x["objtype"] == "CURB").length == 0) {
        ...
        currpattern["verticalstart"] = gtftools.randomInt(1, 2) * 2
      ...
      car["x"] = xMax - 1
      car["y"] = gtftools.randomInt(2,yMax - 3)
      car["objtype"] = "COLLISION"
